...
total-recall is an MCP server providing four-tier memory (Pinned/Hot/Warm/Cold) and a hierarchical knowledge base, all local by default.

**IMMEDIATE ACTION REQUIRED — before responding to the user's first message, call the `session_start` MCP tool.** This syncs host imports, loads pinned directives and hot-tier context, and returns the memory state for the session. If `session_start` has already run, the call returns cached results instantly.

## Session start protocol

1. Call `session_start` before your first reply
2. Announce startup using the returned data (2-3 lines max):
   - Tier summary: pinned, hot, warm, cold, KB counts from `tierSummary`
   ...
   - Example: `total-recall loaded — 2 pinned, 3 hot, 12 warm, 5 cold, 2 KB collections. Storage: sqlite. Last session: 2 hours ago.`
...
### Pinned directives
Pinned directives are re-asserted automatically at each turn by the `UserPromptSubmit` hook. When the user makes a significant task switch, call `session_refresh` once to re-prepend the pinned block.
...
| `session_refresh` | Re-inject pinned block on task switch |
