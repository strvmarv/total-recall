...
describe('TokenUsageCard', () => {
  ...
  it('shows an estimated cost from priced models', async () => {
    const end = Date.UTC(2026, 5, 14);
    vi.spyOn(api, 'tool').mockImplementation((_name: string, args?: unknown) => {
      const gb = (args as { group_by?: string } | undefined)?.group_by;
      if (gb === 'model') return Promise.resolve(usage([{ key: 'claude-sonnet-x', session_count: 1, turn_count: 1, input_tokens: 1_000_000, cache_creation_tokens: 0, cache_read_tokens: 0, output_tokens: 0 }], end)) as Promise<unknown>;
      return Promise.resolve(usage([{ key: '2026-06-10', session_count: 1, turn_count: 1, input_tokens: 1_000_000, cache_creation_tokens: 0, cache_read_tokens: 0, output_tokens: 0 }], end)) as Promise<unknown>;
    });
    render2(<TokenUsageCard refreshKey={0} />);
