...
- **Per-host status:** Claude Code active (`UserPromptSubmit` → `additionalContext`); Copilot CLI wired the same way but upstream currently ignores the returned `additionalContext`; Cursor uses session-start injection + skill-guided `session_refresh` as a layered fallback; Kiro active (`UserPromptSubmit` hook → plain-text STDOUT, `SessionStart` hook → `SKILL.md` injection, `always`-mode steering → persistent `session_start` instruction).
