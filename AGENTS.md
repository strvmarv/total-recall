...
- **Hook:** `hooks/user-prompt-submit/run.sh` is a `UserPromptSubmit` hook (wired in `hooks/hooks.json`). It is a thin wrapper: it picks the host (`claude-code` default; `cursor`/`copilot-cli` from env), short-circuits to `{}` for Cursor (whose `beforeSubmitPrompt` is block-only), and otherwise forwards stdin to the **plugin-bundled** CLI — invoked as `node "$PLUGIN_ROOT/bin/start.js" pinned-floor`, with the plugin root derived from the script's own `${BASH_SOURCE[0]}` location exactly as the MCP server and the `session-start`/`session-end` hooks resolve it, never a bare `total-recall` on `PATH` (which can resolve to a stale or absent global and silently disable the floor). It MUST never fail a prompt — on any non-zero exit or empty output it emits `{}` and exits 0.
