...
- **CLI backend:** `total-recall pinned-floor --host <claude-code|copilot-cli|cursor|kiro>` (`src/TotalRecall.Cli/Commands/PinnedFloorCommand.cs`). Reads the host hook payload (`session_id`, `transcript_path`) from stdin, decides via the throttle, and on an inject turn renders the pinned block live from the DB (via `PinnedBlockRenderer`) and emits host-correct output. For `claude-code`/`copilot-cli` this is a JSON envelope; for `kiro` it is plain text (Kiro appends raw STDOUT verbatim to context, no JSON envelope). Fail-safe: any exception prints `{}` (JSON-envelope hosts) or empty string (`kiro`) and exits 0.
...
- **Per-host status:** Claude Code active (`UserPromptSubmit` → `additionalContext`); Copilot CLI wired the same way but upstream currently ignores the returned `additionalContext`; Cursor uses session-start injection + skill-guided `session_refresh` as a layered fallback; Kiro active (`UserPromptSubmit` command hook → `pinned-floor --host kiro` plain-text STDOUT; `UserPromptSubmit` agent hook → retrieval nudge; `SessionStart` agent hook → `session_start` tool call; `Stop` agent hook → capture; `always`-mode steering → persistent `session_start` instruction). Known limitation: token-growth throttle inactive for Kiro (payload lacks `transcript_path`); floor runs on turn-count only.
