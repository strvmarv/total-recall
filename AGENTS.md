...
- **Per-host status:** Claude Code active (`UserPromptSubmit` → `additionalContext`); Copilot CLI wired the same way but upstream currently ignores the returned `additionalContext`; Cursor uses session-start injection + skill-guided `session_refresh` as a layered fallback; Kiro active (`UserPromptSubmit` command hook → `pinned-floor --host kiro` plain-text STDOUT; `UserPromptSubmit` agent hook → retrieval nudge; `SessionStart` agent hook → `session_start` tool call; `Stop` agent hook → capture; `always`-mode steering → persistent `session_start` instruction). Known limitation: token-growth throttle inactive for Kiro (payload lacks `transcript_path`); floor runs on turn-count only. Gemini CLI active (`BeforeAgent` command hook → `pinned-floor --host gemini-cli` `hookSpecificOutput.additionalContext`; `GEMINI.md` → persistent `session_start` instruction). Token-growth throttle active (payload includes `transcript_path`). Cline/Windsurf/Zed/VS Code: MCP-tools-only (no hooks, no pinned floor); Windsurf has a rules file for session-start instruction.
