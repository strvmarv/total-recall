...
| Project-scoped pin injection | `src/TotalRecall.Infrastructure/Memory/ProjectResolver.cs`, `ProjectKey.cs`, `PinnedScope.cs` |
...
3. **Warm sweep** — if last sweep was more than `warm_sweep_interval_days` ago, moves old unaccessed warm entries to cold. The sweep also runs earned **warm→hot promotion** (an entry with `access_count >= compaction.promote_min_access` AND `decay_score >= compaction.promote_threshold`); sticky entries are excluded from eviction. Tracked via `compaction_log` with `reason = 'warm_sweep_decay'`.
...
6. **Sticky + hot tier assembly** — sticky (pinned) entries are injected first, verbatim, under a `## Pinned directives (always follow)` header (never truncated; their token cost reduces the hot budget), then current non-sticky hot entries as injectable context. Enforces token budget by evicting lowest-decay entries to warm. Sticky entries are never scanned by decay, the warm sweep, or compaction, and are unbounded (exempt from the hot per-entry char cap). The block is rendered by `PinnedBlockRenderer.Render` (`src/TotalRecall.Infrastructure/Memory/PinnedBlockRenderer.cs`) — the single source of truth shared by session start, `session_refresh`, and the per-turn pinned-floor hook. (This rendering was previously inlined as `SessionLifecycle.BuildPinnedBlock`; `SessionLifecycle` now calls the extracted renderer.) **Project scoping** (when `tiers.pinned.project_scoping = true`, the default): `ProjectResolver` detects the current git repo from cwd (pure filesystem walk; no `git` subprocess) and reduces the `[remote "origin"]` URL to a lowercase `owner/repo` slug (or bare folder name when no remote is configured). Pins tagged with a matching `project` value inject only in that repo; untagged pins are global and inject everywhere. When no repo is detected, only global pins inject (fail-closed). Policy is centralised in `PinnedScope.OptsFor` (`src/TotalRecall.Infrastructure/Memory/PinnedScope.cs`).
7. **Tier summary** — counts entries across all tiers and KB collections, returned as `tierSummary` in the response.
8. **Hint generation** — `GenerateHints()` surfaces up to 5 high-value warm memories: corrections and preferences (priority 1), frequently accessed entries with `access_count >= 3` (priority 2), and warm→hot promotion candidates (priority 3). Each hint is truncated to 120 chars. No LLM calls — DB queries only.
...
- **CLI backend:** `total-recall pinned-floor --host <claude-code|copilot-cli|cursor|kiro|gemini-cli>` (`src/TotalRecall.Cli/Commands/PinnedFloorCommand.cs`). Reads the host hook payload (`session_id`, `transcript_path`) from stdin, decides via the throttle, and on an inject turn renders the pinned block live from the DB (via `PinnedBlockRenderer`) and emits host-correct output. For `claude-code`/`copilot-cli`/`gemini-cli` this is a JSON envelope (`hookSpecificOutput.additionalContext`); for `kiro` it is plain text (Kiro appends raw STDOUT verbatim to context, no JSON envelope). Fail-safe: any exception prints `{}` (JSON-envelope hosts) or empty string (`kiro`) and exits 0.
...
- **Config:** the Pinned tier was merged into Hot as a sticky flag in 4.0; `[tiers.pinned]` persists as a deprecated alias whose `floor_*` / `project_scoping` fields still drive sticky re-injection (`max_content_chars` no longer applies). Fields — `floor_enabled` (default `true`), `floor_every_n_turns` (default `6`), `floor_growth_tokens` (default `6000`), `project_scoping` (default `true`). Set `floor_enabled = false` to disable the per-turn floor; set `project_scoping = false` to inject all pins everywhere regardless of repo. Loaded into `Core.Config.PinnedTierConfig` (`FloorEnabled` / `FloorEveryNTurns` / `FloorGrowthTokens` / `ProjectScoping`).
- **Per-host status:** Claude Code active (`UserPromptSubmit` → `additionalContext`); Copilot CLI wired the same way but upstream currently ignores the returned `additionalContext`; Cursor uses session-start injection + skill-guided `session_refresh` as a layered fallback; Kiro active (`UserPromptSubmit` command hook → `pinned-floor --host kiro` plain-text STDOUT; `UserPromptSubmit` agent hook → retrieval nudge; `SessionStart` agent hook → `session_start` tool call; `Stop` agent hook → capture; `always`-mode steering → persistent `session_start` instruction). Known limitation: token-growth throttle inactive for Kiro (payload lacks `transcript_path`); floor runs on turn-count only. Gemini CLI active (`BeforeAgent` command hook → `pinned-floor --host gemini-cli` `hookSpecificOutput.additionalContext`; `SessionStart` command hook → `session_start` instruction; `GEMINI.md` → persistent `session_start` instruction). Token-growth throttle active (payload includes `transcript_path`). Cline/Windsurf/Zed/VS Code: MCP-tools-only (no hooks, no pinned floor); Windsurf has a rules file for session-start instruction.
